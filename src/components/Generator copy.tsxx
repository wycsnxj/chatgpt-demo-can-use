...
//       // *********** 强制转换 response.body 的类型为 NodeJS.ReadableStream
//       data.pipe(rs).pipe((err, result) => { // *********** 修改这一行
//         if (err) {
//           console.error(err);
//         } else {
//           // *********** 处理可写流的返回值，比如打印结果或者触发事件等
//           console.log(result); // 打印结果
//           setCurrentAssistantMessage(result); // 设置结果为当前助手消息
//           smoothToBottom(); // 平滑滚动到底部
//           archiveCurrentMessage(); // 存档当前消息
//           // *********** 修改以上代码
//         }
//       });
